# Blueberry Template - 프로젝트 컨벤션

이 프로젝트는 **Flutter + Riverpod 3.0** 기반의 클린 아키텍처 템플릿입니다.

## 핵심 원칙

1. **심플함 유지**: 과도한 추상화 없이 명확하고 직관적인 코드
2. **타입 안정성**: 강타입 사용, null safety 준수
3. **문서화 언어**: 기존 파일의 주석 언어를 따라가기 (한국어 파일은 한국어, 영어 파일은 영어)
4. **const 최대 활용**: 컴파일 타임 상수로 성능 최적화
...
lib/
├── core/
│   ├── controllers/    # 전역 상태 관리 (테마, 언어 등)
│   └── themes/         # 디자인 시스템
└── features/           # 기능별 모듈
    └── [feature_name]/
        ├── controllers/  # 상태 관리 (Riverpod Notifier)
        ├── models/       # 데이터 모델
        ├── screens/      # UI 스크린
        └── widgets/      # 해당 기능 전용 위젯
```
